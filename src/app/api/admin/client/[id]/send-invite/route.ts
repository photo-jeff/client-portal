...
export async function POST(request: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  const partner1First = client.partner1_name?.trim().split(/\s+/)[0] ?? client.partner1_name ?? 'there'
  ...
  const html = `
    <div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;color:#1a1a1a;background:#faf9f7;padding:40px 32px">
      ...
      <h1 style="font-size:28px;font-weight:400;margin:0 0 16px;line-height:1.3">
      ...
      <p style="font-size:12px;color:#bbb;line-height:1.6">
        Jeff Oliver Photography · London &amp; Surrey Weddings
      </p>
    </div>
  `
  ...
  const emailRes = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'Jeff Oliver Photography <portal@jeffoliverphotography.com>',
      to: client.email,
